...
# ─────────────────────────────────────────────
echo ""
echo "=== Test 3: Empty phonebook ==="
OUT=$(run "$(printf 'SEARCH\n999\nEXIT')")
assert_contains "invalid index error" "$OUT" "Phonebook empty! Search only after adding at least on contact."  # adjust to match your program's actual message
...
# ─────────────────────────────────────────────
echo ""
echo "=== Test 6: Invalid index search ==="
OUT=$(run "$(printf 'ADD\nJohn\nDoe\nJD\n123456789\nMy first contact\nSEARCH\n999999999999999\n')")
assert_contains "invalid index error" "$OUT" "Index out of range!"
